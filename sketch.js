...
function setup() {
  ...
  trex = createSprite(40,height-180,20,50);
  ...
  restart = createSprite(width/2,length+370);
  ...
  invisibleGround = createSprite(40,height,400,10);
...
function draw() {
  ...
  background(30);
  fill('white');
  text("Puntuación: "+ score, width/2,height/3);
  
  if (gameState===PLAY){
    score = score + Math.round(getFrameRate()/60);
    ground.velocityX = -(6 + 3*score/100);
  
    if(keyDown("space") && trex.y >= width/1.15 ) {
      trex.velocityY = -12;
    }
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(600,height/1.03,10,40);
    //obstacle.debug = true;
